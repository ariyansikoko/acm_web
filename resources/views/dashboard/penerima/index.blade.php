...
@section('body')
    ...
    <div class="table-responsive small col-md-4">
            ...
            <tbody>
                @foreach ($recipient as $post)
                    <tr>
                        ...
                        <td>{{ $post->name }}</td>
                        <td>
                            </a> <a href="/dashboard/penerima/{{ $post->slug }}/edit" class="btn btn-warning btn-sm">
                                <i class="bi bi-pencil-square"></i></a>
                            <form action="/dashboard/penerima/{{ $post->slug }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
